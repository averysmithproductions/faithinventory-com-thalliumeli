                ...
                if (err) {
                    ...
                    if (Item) {
                        ...
                        const emailData = {
                            ...
                            to: email,
                            magicLink,
                            subject: `Your FaithInventory Magic Link to Sign In Will Expire in ${hoursUntilExpiration} Hours`,
                            body: `Sign in with your FaithInventory Magic Link below (the link expires in ${hoursUntilExpiration} hours).`
                        }
    ...
    if(event.headers['Authorization']) {
        ...
        VerifyHash(event, (error, response) => {
            ...
            if( response.statusCode === 200 ) {
                ...
                console.log('D')
                const {
                    PLATINUMENOCH_MEDIA_BUCKET_NAME
                } = process.env
                ...
                for (let i = 0; i < urlAmount; i++) {
                    const id = uuidv4()
                    const s3Params = {
                        Bucket: PLATINUMENOCH_MEDIA_BUCKET_NAME,
...
const invalidateCloudFrontCache = paths => {
    const { BARIUMNAHUM_DISTRIBUTION_ID } = process.env
    var params = {
